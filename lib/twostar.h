...
int twostar(int root,int *groupIds,int *onestar,int numGroups,int remGroups,int V, int *D, int *partialStar1,int *intermSet,int *newGroupIds, bool buildSolution) {		
	int TOTAL_COST = 0;
	remGroups = numGroups;
	...
	if(!buildSolution) { //if building a solution we already run twostarrupper amd got the total cost
		return TOTAL_COST;
	}
	else {
		return c;
...
void twostarwrapper(int V, int numGroups, int perChild, int perParent, int numProc, int procId, int * D, int * onestar, struct Solution *solution) {
	...
	for(int i = 0; i < perChild; i++) {
		...
		TOTAL_COST = twostar(root,groupIds,onestar,numGroups,remGroups,V,D, partialStar1,intermSet,newGroupIds, false);		
